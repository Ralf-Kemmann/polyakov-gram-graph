...
def fairness_overlap_slices(W_tau: int, delta: int) -> Tuple[slice, slice]:
    """
    Fair support length L = W_tau - |delta|, but compare SAME local indices in both windows.
    This avoids the trivial identity K_ij=1 caused by comparing the physical overlap region.
    """
    L = W_tau - abs(delta)
    if L <= 0:
        return slice(0, 0), slice(0, 0)
    return slice(0, L), slice(0, L)
